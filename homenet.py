...
@homenet_bp.route("/api/homenet/backup/health")
def homenet_backup_health():
    """Backup staleness snapshot for the dashboard concerns pipeline.

    Returns the age of the newest backup per vendor and whether it
    has crossed the staleness threshold. There is no scheduler --
    backups are manual via the device admin Save/Restore page (see
    the Backup buttons in the Home Network tab).
    """
    return jsonify(get_backup_health())
...
# Earlier iteration (reverted 2026-05-11): a daemon thread fired
# _orbi_backup_config every 24h. Reverted because the Orbi endpoint
# returns HTTP 500 on RBRE960 firmware (the documented pynetgear path
# doesn't work; no public implementation found via GitHub code search;
# live-probing the right endpoint requires the user's browser DevTools).
# Verizon never supported auto-backup (deep-link needs browser
# interaction). So the scheduler was firing daily failures with nothing
# to show for it -- user feedback: "if both backups don't work why even
# do it?".
#
# Current honest design: backups are MANUAL. The Backup buttons in
# the UI open the device's admin Save/Restore page in a new tab;
# user clicks Save there. To track staleness, get_backup_health()
# reports the age of the newest file in each vendor's directory and
# the dashboard surfaces a concern when it crosses the per-vendor
# threshold.

_BACKUP_VERIZON_STALE_DAYS = 30
_BACKUP_ORBI_STALE_DAYS = 30
...
def get_backup_health() -> dict:
    ...
    out: dict = {
        ...
        "orbi_stale_threshold_days": _BACKUP_ORBI_STALE_DAYS,
    }
    for vendor in ("orbi", "verizon"):
        ...
        threshold = _BACKUP_VERIZON_STALE_DAYS if vendor == "verizon" else _BACKUP_ORBI_STALE_DAYS
