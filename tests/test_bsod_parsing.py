...
class TestParseEvent:
    ...
    def test_1001_missing_bugcheck_line_returns_none(self):
        ...
        assert bsod.parse_event(evt) is None
    ...
    def test_empty_event_returns_none(self):
        assert bsod.parse_event({}) is None
...
class TestParseReportCrashes:
    ...
    def test_known_error_code_extracted(self, tmp_path):
        ...
        results = bsod.parse_report_crashes(path)
    ...
    def test_nonexistent_file_returns_empty(self, tmp_path):
        results = bsod.parse_report_crashes(str(tmp_path / "missing.html"))
        assert results == []
...
class TestBuildRecommendations:
    def test_zero_crashes_system_stable(self):
        recs = bsod.build_recommendations([])
        assert len(recs) == 1
    ...
    def test_more_than_ten_crashes_critical_rec(self):
        ...
        recs = bsod.build_recommendations(crashes)
