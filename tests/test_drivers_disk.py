...
class TestPoolRepair:
    """Replace-and-rebuild driver: non-elevated plan, elevated start, status."""

    def _spaces(self, retired=1, pools=1):
        members = [{"Name": "Samsung 990 PRO 2TB", "Serial": "0025_29B7.", "Usage": "Auto-Select", "Operational": "OK"}]
        for i in range(retired):
            members.append(
                {"Name": "Samsung 990 PRO 2TB", "Serial": f"0025_20C{i}.", "Usage": "Retired", "Operational": "OK"}
            )
        return {
            "pools": [{"Name": "Storage pool"}] * pools,
            "virtual_disks": [{"Name": "Storage space", "Resiliency": "Parity"}] if pools else [],
            "members": members,
        }

    def _candidates(self, mocker, items):
        m = mocker.patch("disk.subprocess.run")
        m.return_value.stdout = json.dumps(items) if items is not None else ""
        m.return_value.returncode = 0
        m.return_value.stderr = ""
        return m

    # ── plan ──────────────────────────────────────────────────────────────
    def test_plan_ready_with_one_retired_and_one_candidate(self, mocker):
        mocker.patch("disk.get_storage_spaces", return_value=self._spaces())
        self._candidates(mocker, [{"Name": "New 990 PRO 2TB", "Serial": "NEW1.", "SizeGB": 1863}])
        p = disk.get_pool_repair_plan()
        assert p["ready"] is True
        assert p["blockers"] == []
        assert p["remove"]["serial"] == "0025_20C0"
        assert p["add"]["serial"] == "NEW1"

    def test_plan_blocks_when_no_replacement_fitted(self, mocker):
        mocker.patch("disk.get_storage_spaces", return_value=self._spaces())
        self._candidates(mocker, [])
        p = disk.get_pool_repair_plan()
        assert p["ready"] is False
        assert any("No new poolable disk" in b for b in p["blockers"])

    def test_plan_blocks_on_multiple_candidates(self, mocker):
        mocker.patch("disk.get_storage_spaces", return_value=self._spaces())
        self._candidates(mocker, [{"Name": "A", "Serial": "A."}, {"Name": "B", "Serial": "B."}])
        p = disk.get_pool_repair_plan()
        assert p["ready"] is False
        assert any("2 poolable disks" in b for b in p["blockers"])

    def test_plan_blocks_when_nothing_retired(self, mocker):
        mocker.patch("disk.get_storage_spaces", return_value=self._spaces(retired=0))
        self._candidates(mocker, [{"Name": "New", "Serial": "NEW1."}])
        p = disk.get_pool_repair_plan()
        assert p["ready"] is False
        assert p["remove"] is None
        assert any("nothing to replace" in b.lower() for b in p["blockers"])

    def test_plan_blocks_on_multiple_retired(self, mocker):
        mocker.patch("disk.get_storage_spaces", return_value=self._spaces(retired=2))
        self._candidates(mocker, [{"Name": "New", "Serial": "NEW1."}])
        p = disk.get_pool_repair_plan()
        assert p["ready"] is False
        assert any("2 retired/missing" in b for b in p["blockers"])

    def test_plan_no_pool_is_safe(self, mocker):
        mocker.patch("disk.get_storage_spaces", return_value={"pools": [], "virtual_disks": [], "members": []})
        p = disk.get_pool_repair_plan()
        assert p["ready"] is False

    # ── start ─────────────────────────────────────────────────────────────
    def test_start_launches_elevated_with_derived_targets(self, mocker):
        mocker.patch("disk.get_storage_spaces", return_value=self._spaces())
        launch = mocker.patch("disk._launch_elevated_ps", return_value={"ok": True})
        out = disk.start_pool_repair()
        assert out["ok"] is True and out["started"] is True
        script, args = launch.call_args.args
        assert script.endswith("repair_storage_pool.ps1")
        # targets come from Windows, not the caller
        assert '-PoolName "Storage pool"' in args
        assert '-VirtualDiskName "Storage space"' in args
        assert "0025_20C0" in args
        assert "-Preview" not in args

    def test_start_preview_passes_preview_flag(self, mocker):
        mocker.patch("disk.get_storage_spaces", return_value=self._spaces())
        launch = mocker.patch("disk._launch_elevated_ps", return_value={"ok": True})
        disk.start_pool_repair(preview=True)
        assert "-Preview" in launch.call_args.args[1]

    def test_start_refuses_when_nothing_retired(self, mocker):
        mocker.patch("disk.get_storage_spaces", return_value=self._spaces(retired=0))
        launch = mocker.patch("disk._launch_elevated_ps")
        out = disk.start_pool_repair()
        assert out["ok"] is False
        launch.assert_not_called()

    def test_start_refuses_on_multiple_retired(self, mocker):
        mocker.patch("disk.get_storage_spaces", return_value=self._spaces(retired=2))
        launch = mocker.patch("disk._launch_elevated_ps")
        out = disk.start_pool_repair()
        assert out["ok"] is False
    ...
    def test_start_surfaces_declined_elevation(self, mocker):
        mocker.patch("disk.get_storage_spaces", return_value=self._spaces())
        mocker.patch("disk._launch_elevated_ps", return_value={"ok": False, "error": "Elevation declined (rc=5)."})
        out = disk.start_pool_repair()
        assert out["ok"] is False
        assert "Elevation declined" in out["error"]

    def test_start_sanitises_targets_into_args(self, mocker):
        # Windows-sourced strings are still scrubbed before hitting a cmdline.
        spaces = self._spaces()
        spaces["pools"][0]["Name"] = 'Pool"; Remove-Item C:\\ -Recurse #'
        mocker.patch("disk.get_storage_spaces", return_value=spaces)
        launch = mocker.patch("disk._launch_elevated_ps", return_value={"ok": True})
        disk.start_pool_repair()
        args = launch.call_args.args[1]
        assert "Remove-Item" not in args or ";" not in args
        assert '"' not in args.split("-PoolName ")[1].split(" -VirtualDiskName")[0].strip('"')

    # ── status ────────────────────────────────────────────────────────────
    def test_status_idle_when_no_run(self, mocker, tmp_path):
        mocker.patch.object(disk, "POOL_REPAIR_STATE_FILE", str(tmp_path / "none.json"))
        s = disk.get_pool_repair_status()
        assert s["ok"] is True and s["stage"] == "idle"

    def test_status_passes_through_state(self, mocker, tmp_path):
        f = tmp_path / "state.json"
        f.write_text(json.dumps({"stage": "repairing", "percent": 42, "message": "Rebuilding", "done": False}))
        mocker.patch.object(disk, "POOL_REPAIR_STATE_FILE", str(f))
        s = disk.get_pool_repair_status()
        assert s["stage"] == "repairing" and s["percent"] == 42

    def test_status_malformed_is_safe(self, mocker, tmp_path):
        f = tmp_path / "bad.json"
        f.write_text("not json{")
        mocker.patch.object(disk, "POOL_REPAIR_STATE_FILE", str(f))
        s = disk.get_pool_repair_status()
        assert s["ok"] is True and s["stage"] == "idle" and "read_error" in s
